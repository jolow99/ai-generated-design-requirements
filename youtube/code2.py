...
def code2(): 
    ...
    for i in range(len(data)):                                           # 'i' is a page (a row in df)
        for j in range(len(data[i][2])):                                 # 'j' is a comment on the page
            ...
            for k in range(len(tokens)):
                tokens[k] = WordNetLemmatizer().lemmatize(tokens[k])      # stem each token
    ...
    data_clean.to_csv("support/%s/data_clean.csv" % search_terms)
    print(data_clean)
    ...
    dtm = pd.DataFrame(data_cv.toarray(), columns=cv.get_feature_names(), index=pd.Series(vid_title))
    print(dtm)
    ...
    # Turn clean_comments into dataframe then output as a csv
    clean_comments_df = pd.DataFrame(clean_comments)
    clean_comments_df.to_csv("support/%s/clean_comments.csv" % search_terms)
